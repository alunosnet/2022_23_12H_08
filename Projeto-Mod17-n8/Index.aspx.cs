...
namespace Projeto_Mod17_n8
{
    public partial class Index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //atualizar grelha roupas
            atualizaListaRoupas(null, ordenar);
        }

        private void atualizaListaRoupas(string pesquisa = null, int? ordena = null)
        {
            ...
            if (pesquisa == null)
            {
                ...
                if (httpCookie == null)
                    dados = livro.listaRoupasDisponiveis(ordena);
                else
                {
                    dados = livro.listaLivrosDaMarca(Server.UrlDecode(httpCookie.Value));
                    dados.Merge(livro.listaRoupasDisponiveis(ordena));
                }
            }
            else
            {
                dados = livro.listaRoupasDisponiveis(pesquisa, ordena);
            }
            gerarIndex(dados);
        }
        ...
        private void gerarIndex(DataTable dados)
        {
            ...
            foreach (DataRow livro in dados.Rows)
            {
                ...
                grelha += "<img height='180' src='/Public/Imagens/" + livro[0].ToString() +
                    ".jpg' class='img-fluid'/>";
                grelha += "<p/><span class='stat-title'>" + livro["tiporoupa"].ToString()
                    + "</span>";
                grelha += "<p/><span class='stat-title'>" + livro["marca"].ToString()
                    + "</span>";
                grelha += "<p/><span class='stat-title'>" + livro["genero"].ToString()
                    + "</span>";
                grelha += "<span class='stat-title'>" +
                    String.Format(" | {0:C}", Decimal.Parse(livro["preco"].ToString()))
                    + "</span>";
                grelha += "<br/><a href='detalhesroupa.aspx?id=" + livro[0].ToString()
                    + "'>Detalhes</a>";
                grelha += "</div>";
            }
        ...
        {
            atualizaListaRoupas(tbPesquisa.Text);
        }
    }
}